...
Only the team deliverable lives here. Individual reports are personal
submissions, graded and moderated per member, so they are not committed to the
shared repository — `.gitignore` blocks them. The presentation slides, the pitch
video and the peer evaluation are also submitted outside the repository; their
links are on the group report's *Auxiliary Resources* page.
