...
3. **Service** — `createTicket` ([ticket.service.ts:10-34](server/services/ticket.service.ts#L10-L34)):
   ...
   - Then the two UC8 emails inside a `try/catch`: confirmation to the submitter and a notification to `ADMIN_EMAIL` (the Sprout team inbox, `sproutteamadmin@gmail.com`). **The catch is the use case's alt-flow 5a**: if email delivery fails, it logs `"email delivery failed ... logged for manual retry"` and the request still succeeds — your ticket is never lost to an email outage.
...
| **Contact/query ticket** | Implemented with contract differences | The route is `POST /api/query/submit`, not `POST /api/query` ([query.routes.ts:19](../server/routes/query.routes.ts#L19)). The form sends `name`, `email`, `category`, and `message`; it does not collect the diagram's optional organisation or subject fields, and `category` replaces `inquiryType` ([ContactPage.tsx:11](../client/src/pages/ContactPage.tsx#L11), [ticket.ts:13](../server/models/ticket.ts#L13)). The ticket is persisted before email delivery and receives an `SPR-YYYYMMDD-NNNN` reference. Email failure is caught and console-logged for manual retry, but there is no persisted retry job/queue. Because submitter confirmation and team notification are sequential calls inside one `try`, failure of the first email also skips the team-notification call ([ticket.service.ts:11](../server/services/ticket.service.ts#L11)). |
