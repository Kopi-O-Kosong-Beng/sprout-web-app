...
- [x] 1.4 Scaffold `server/` with `package.json`, `app.js`, and the full directory structure: `routes/`, `controllers/`, `services/`, `middleware/`, `models/`, `database/`, `utils/`, `tests/`
...
- [ ] 2.2 Create `server/database/db.js` that initialises and exports a SQLite connection with WAL mode
...
- [~] 2.7 Create `server/database/seed.js` that inserts sample users and avatar records for local development and testing
...
- [~] 3.1 Create `server/middleware/auth.middleware.js` — verifies JWT from `Authorization: Bearer` header, attaches `req.user`, returns 401 on failure
- [~] 3.2 Create `server/middleware/validation.middleware.js` — generic Joi/Zod schema validation wrapper
- [~] 3.3 Create `server/services/email.service.js` — Nodemailer SMTP wrapper; in dev mode logs emails to console instead of sending
- [~] 3.4 Create `server/controllers/auth.controller.js` with `handleSignup`: validate input, check duplicate email, hash password (bcrypt cost 12), create user, send verification email, respond 201
- [~] 3.5 Add `handleVerifyEmail` to `auth.controller.js`: find user by token, set `isVerified=true`, respond 200
- [~] 3.6 Add `handleLogin` to `auth.controller.js`: validate credentials, check `isVerified`, sign JWT (1h expiry), respond 200 with token + displayName
- [~] 3.7 Add `handleLogout` to `auth.controller.js`: invalidate JWT via in-memory or DB token blacklist, respond 200
- [~] 3.8 Create `server/routes/auth.routes.js` and wire all auth endpoints with rate-limit middleware (10 attempts / 15 min / IP)
- [~] 3.9 Mount auth router in `server/app.js` at `/api/auth`

## Task 4: Password Reset via OTP
Implement OTP-based password reset per Requirement 3.

- [~] 4.1 Add `handleRequestReset` to `auth.controller.js`: generate 6-digit OTP via `crypto.randomInt`, hash with bcrypt, store hash + 15-min TTL on user record, send OTP email, respond 200 (even if email not found)
- [~] 4.2 Add `handleVerifyReset` to `auth.controller.js`: look up user, check OTP TTL, compare OTP hash, validate new password strength, reject passwords found in `password_history` (Req 3.9), archive the old hash to `password_history` (Req 3.10), update password hash, clear OTP fields, respond 200
- [~] 4.3 Add `POST /request-reset` and `POST /verify-reset` to `auth.routes.js`

## Task 5: Avatar Archive Backend
Implement avatar listing and detail routes per Requirement 5.

- [~] 5.1 Create `server/controllers/avatar.controller.js` with `handleListAvatars`: verify JWT, query DB for user's avatars with pagination (page/pageSize query params, default 20), respond 200
- [~] 5.2 Add `handleGetAvatar` to `avatar.controller.js`: verify ownership, respond with single AvatarRecord or 404
- [~] 5.3 Create `server/routes/avatar.routes.js` with `GET /` and `GET /:avatarId` (all protected by auth middleware)
- [~] 5.4 Mount avatar router in `server/app.js` at `/api/avatar`

## Task 6: GenAI Upload Pipeline Backend
Implement the plant image upload and GenAI pipeline per Requirements 6 and 7.

- [~] 6.1 Create `server/services/plantId.service.js` — wraps HTTP call to PlantID API (or mock); timeout 10s; retry 2×; returns `{ species, confidence, taxonomy }`
- [~] 6.2 Create `server/services/gemma.service.js` — wraps HTTP call to Gemma API (or mock); timeout 15s; retry 1×; returns `{ prompt }`
- [~] 6.3 Create `server/services/flux.service.js` — wraps HTTP call to FLUX API (or mock); timeout 30s; retry 1×; returns `{ spriteUrl }`
- [~] 6.4 Create `server/utils/avatarStats.js` — implements `deriveAvatarStats(speciesName, taxonomy)` as a pure function using deterministic hash + taxonomy modifiers per design algorithm
- [~] 6.5 Create `server/middleware/upload.middleware.js` — Multer config: memory storage, 5 MB limit, JPEG/PNG/WEBP only (magic bytes check)
- [~] 6.6 Create `server/controllers/upload.controller.js` with `handlePlantUpload`: validate file, call PlantID, check confidence threshold, call Gemma, call FLUX, derive stats, persist TempAvatar (isTemporary=true, TTL 24h), respond 200 with TempAvatarResult
- [~] 6.7 Add rollback logic: if any pipeline step fails, delete any partial DB record and respond 503
- [~] 6.8 Create `server/routes/upload.routes.js` with `POST /plant` (auth + upload middleware + rate limit 5/hr/user)
- [~] 6.9 Mount upload router in `server/app.js` at `/api/upload`
- [~] 6.10 Create mock service responses in `server/utils/mockServices.js` for use when `USE_MOCK_APIS=true` in `.env`

## Task 7: PVE Battle Backend
Implement the PVE battle service and routes per Requirement 8.

- [~] 7.1 Create `server/services/battle.service.js` with pure function `calculateDamage(attacker, defender, action)` — implement attack/special/defend logic, minimum 1 damage, deterministic
- [~] 7.2 Add `generateNpc(userAvatarStats)` to `battle.service.js` — creates a balanced NPC opponent
- [~] 7.3 Add `selectNpcAction(npcStats, npcCurrentHp)` to `battle.service.js` — heuristic: use special when HP > 70%, defend when HP < 30%, else attack
- [~] 7.4 Add `resolveBattleTurn(session, userAction)` to `battle.service.js` — orchestrates user move, NPC counter-move, HP floor at 0, win/loss detection, log appending
- [~] 7.5 Create `server/controllers/battle.controller.js` with `handleStartPve`: load avatar (or temp avatar), generate NPC, create BattleSession in DB, respond with initial state
- [~] 7.6 Add `handleBattleAction` to `battle.controller.js`: load session, verify ownership + active status, call `resolveBattleTurn`, save session, respond with updated state
- [~] 7.7 Add `handleGetSession` to `battle.controller.js`: return current session state for GET endpoint
- [~] 7.8 Create `server/routes/battle.routes.js` with `POST /pve/start`, `POST /pve/action`, `GET /pve/:sessionId` (all protected)
- [~] 7.9 Mount battle router in `server/app.js` at `/api/battle`

## Task 8: Query Ticket Backend
Implement the query ticket creation and email notification per Requirement 9.

- [~] 8.1 Create `server/services/ticket.service.js` with `createTicket(data)`: validate category enum, generate unique `RefNumber` (`SPR-YYYYMMDD-NNNN` with atomic daily counter), persist QueryTicket, send confirmation + admin notification emails, return ticket
- [~] 8.2 Create `server/controllers/query.controller.js` with `handleQuerySubmit`: validate input (name, email, category, message ≤ 2000 chars), call ticket service, respond 201 with refNumber
- [~] 8.3 Create `server/routes/query.routes.js` with `POST /submit` (public route, no auth required)
- [~] 8.4 Mount query router in `server/app.js` at `/api/query`

## Task 9: Express App Setup & Global Middleware
Wire all middleware, routers, and error handling into `server/app.js`.

- [~] 9.1 Configure `cors` in `server/app.js` to allow `http://localhost:5173` in development
...
- [~] 9.4 Create `server/middleware/error.middleware.js` — catches errors from all routes, returns appropriate HTTP status and JSON error body
- [~] 9.5 Add a `GET /api/health` route that returns `{ status: "ok", timestamp }` for dev verification
- [~] 9.6 Create `server/server.js` entry point that imports `app.js`, runs migrations, and starts listening on `PORT` (default 3001)
...
- [~] 18.1 Create `server/tests/auth.test.js` — test signup validation (valid, duplicate email, weak password, malformed email), login (valid, wrong password, rate limit simulation), OTP flow (generate, verify, expired, invalid)
- [~] 18.2 Create `server/tests/upload.test.js` — test file format validation, size validation, PlantID mock success/failure/low-confidence, Gemma mock, FLUX mock, pipeline rollback on failure
- [~] 18.3 Create `server/tests/battle.test.js` — test `calculateDamage` (attack/special/defend), `generateNpc`, `selectNpcAction`, `resolveBattleTurn` (win/loss detection, HP floor), `deriveAvatarStats` range bounds
- [~] 18.4 Create `server/tests/query.test.js` — test ticket creation, RefNumber format, duplicate prevention, field validation (missing fields, long message, invalid category)
- [~] 18.5 Create `server/tests/avatar.test.js` — test avatar list retrieval (paginated, empty), ownership check (other user's avatar returns 403/404)
...
- [~] 19.1 Create `server/tests/pbt/avatarStats.pbt.test.js` — Property 1 (stats within bounds for all species/taxonomy combos) + Property 2 (determinism)
- [~] 19.2 Create `server/tests/pbt/battleDamage.pbt.test.js` — Property 3 (damage ≥ 1 and deterministic for attack/special) + Property 4 (HP never drops below 0 across action sequences)
- [~] 19.3 Create `server/tests/pbt/ticketRef.pbt.test.js` — Property 5 (unique RefNumbers for n submissions per day)
- [~] 19.4 Create `server/tests/pbt/auth.pbt.test.js` — Property 7 (passwords never stored in plaintext) + Property 8 (invalid JWT always returns 401)
...
- [~] 21.1 Create `server/tests/integration/auth.integration.test.js` — Signup → verify email → login → receive JWT → access protected route
- [~] 21.2 Create `server/tests/integration/avatar.integration.test.js` — Login → GET avatar archive → verify avatars match seeded DB records
- [~] 21.3 Create `server/tests/integration/pve.integration.test.js` — Login → select avatar → start PVE session → submit 3 actions → session concludes
- [~] 21.4 Create `server/tests/integration/upload.integration.test.js` — Login → upload plant (mocked pipeline) → receive TempAvatar → use in PVE
- [~] 21.5 Create `server/tests/integration/query.integration.test.js` — Submit ticket → ticket created → refNumber returned → confirmation email logged
